...
import { Trash2, Users, Settings, LayoutDashboard, ChevronLeft, ChevronRight, PlusCircle, Download, List } from 'lucide-react'
              ...
              <div className="flex items-center gap-4">
                ...
                <span className="font-black text-2xl text-slate-800">
                  {viewMode === 'month' 
                    ...
                    : `${selectedDate.getMonth()+1}月${selectedDate.getDate()}日の週`
                  }
                </span>
              </div>
              ...
              {viewMode === 'month' ? (
                ...
                <div className="grid grid-cols-7 gap-px bg-slate-200 border rounded-2xl overflow-hidden shadow-inner font-bold">
                  ...
                  {[0,1,2,3,4,5,6].map(i => {
                    ...
                    return (
                      <div key={i} onClick={()=>setSelectedDate(d)} className={`bg-white p-1 min-h-[300px] cursor-pointer transition-colors ${d.toDateString()===selectedDate.toDateString()?'bg-blue-50 ring-2 ring-inset ring-blue-400':''}`}>
          ...
          <div className="bg-white p-6 rounded-[1.5rem] shadow-xl border border-slate-100 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-3 max-h-[450px] overflow-y-auto pr-1">
              {selectedDayShifts.length === 0 ? (
                <p className="text-xs text-slate-400 text-center py-4 italic">この日のシフトはありません</p>
              ) : (
                selectedDayShifts.map(s => (
                  <div key={s.id} className={`p-4 rounded-2xl border-2 flex flex-col gap-2 shadow-sm ${getStaffColor(s.staff_name).replace('bg-', 'border-')}`}>
                    ...
                    <div className="flex gap-2">
                      <select 
                        className="flex-1 border p-2 rounded-xl text-[10px] font-bold bg-white outline-none"
